...
请注意，此处提到的配置文件为仓库根目录下的 `_config.kratos-rebirth.yml` 主题配置文件。如果错误地将配置追加到 *站点配置文件* 则不会生效，且可能会导致出现一些意想不到的问题。

" %}

主题的配置分为以下几大部分：系统配置、重要功能配置、样式配置、娱乐功能配置、评论与访问量统计、额外的注入代码，以下是分别的解释。

注意这几个层级并属于配置文件的层级结构，而是语义化的分类整理。这篇文档主要也是语义化的解释，具体到每一项精确的控制可以参考 [底层配置文件](https://github.com/Candinya/Kratos-Rebirth/blob/master/_config.yml) 里的注释内容。
